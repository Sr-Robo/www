...
import { presentCharge } from '../../services/presentCharge.js';
...
export default async (request: any, response: any, next: any) => {
  ...
  if (!order || order.payment_method !== 'pix' || order.payment_status !== 'pending') return response.status(INVALID_PAYLOAD).json({ error: { status: INVALID_PAYLOAD, message: 'Invalid PIX order' } });
  const existing = await select().from('pix_payment_attempt').where('order_id', '=', order.order_id).load(pool);
  if (existing) return response.status(OK).json({ data: await presentCharge(existing) });
  ...
  response.status(OK).json({ data: await presentCharge({ provider_reference: providerReference, qr_code: copyPaste, copy_paste: copyPaste, status: 'waiting', expires_at: expiresAt }) });
};
