...
export async function confirmPixPayment(providerReference: string, endToEndId: string) {
  ...
  try {
    ...
    if (!attempt) throw new Error('PIX charge not found');
    if (attempt.status === 'paid') { await commit(connection); return attempt; }
    if (attempt.status !== 'waiting') throw new Error(`PIX charge cannot be confirmed from ${attempt.status}`);
    if (new Date(attempt.expires_at).getTime() < Date.now()) throw new Error('PIX charge expired');
    const order = await select().from('order').where('order_id', '=', attempt.order_id).load(connection);
    if (!order || order.payment_method !== 'pix' || order.payment_status !== 'pending') throw new Error('Order is not payable by PIX');
